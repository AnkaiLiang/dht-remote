...
public class WebClient {
	...
	public NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws DHTBase.Failed {
		// TODO Auto-generated method stub
		UriBuilder ub = UriBuilder.fromUri(info.addr).path("closestPrecedingFinger");
	...
	public void add(NodeInfo n, String k, String v) throws DHTBase.Failed {
		...
		TableRep table_rep = new TableRep(null, null, 1);
		table_rep.entry[0] = new TableRow(k, new String[]{v});
		info("client add(" + addPath + ")");
		Response response = putRequest(addPath, Entity.xml(table_rep));
		if (response == null || response.getStatus() >= 300) {
